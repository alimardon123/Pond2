...
| `docs/builder-spec-c17.md` | — | **N+6 C17 builder spec** (task cron-2026-08-30-a): the substrate builder's contract for the get_path error-channel refactor — `io::Result<Option<String>>` across the trait + LocalFS/S3/Caching backends (NotFound/404 discrimination, no cache poisoning), `PondKernel::resolve` + the 112-caller sweep, fallible `probe_writer` (a failed epoch probe is a truncated view — error, not empty suffix), the pyo3 `PyResult` surface, and REVERT-VERIFIED ref-outage tests. |
| `docs/builder-spec-pnps-laws.md` | — | **N+6 C12 builder spec** (task cron-2026-08-30-b): the codec builder's contract for the PNPK + PSLB property suites (`laws_pnps.rs`) — 5 pack laws (lossless round-trip, magic discrimination, truncation rejection, fuzz-no-panic, determinism) + 6 slab laws (lossless round-trip, tail invariant, range-fetch reconstruction, compressed round-trip, truncation discrimination, conservative plan_ranges pruning), following the laws_pman patterns. |
...
| `core/storage/src/read.rs` | — | Read paths: read (N+6 C13: journal-AWARE raw reader — resolves the D6 plan, concatenates live RG bytes), read_rows_i64 (pruned i64 pipeline), read_rows_json_pruned (journal-aware: snapshot ∪ live entries, per-pack pruned pipeline + CRDT merge), read_rows_json_pruned_with_head (pure per-pack reader), read_all_row_groups, indexed lookups. |
| `core/storage/tests/journal_test.rs` | — | D3 journal integration harness (29 tests): history-preservation P0 probe (C9), N sequential writes, 8×8 concurrent writers (no lost rows), C10 permutation determinism, warm-read zero-LIST budget (CountingStore), legacy-repo compat, compaction folds entries+shards (tombstone-safe, benign race, upto watermark), journal status, shared-ref immutability of appends, TTL=0 fresh-list env knob (child-process), deterministic entry order, C17 probe-outage (entry-read outage → resolve_packs Err, not a truncated view; recovery → complete), C13 raw-read journal-awareness (second write visible pre-fold; entries-only bootstrap window reads entries). |
| `core/storage/tests/laws_pnps.rs` | — | **N+6 C12 codec laws**: 11 proptest laws + adversarial companions for PNPK (pond_pack) and PSLB (slab) — lossless round-trips, magic discrimination, truncation rejection, fuzz-no-panic, determinism, PSLB tail invariant, range-fetch reconstruction, compressed round-trips, conservative plan_ranges pruning. Found + recorded FINDING #1 (serde_json 1-ULP float loss → float_roundtrip feature enabled, law un-ignored) and FINDING #2 (decode_slab_footer 192 GiB alloc abort → n_entries ≤ footer_len/21 validation, test un-ignored). |
...
| `cli/Cargo.toml` | 20 | CLI binary crate. Produces the `pond` executable. Dev-deps include pond_kernel (blob seeding for C20 cat tests). |
| `cli/src/main.rs` | 2155 | The `pond` CLI: init, write (JSON/file/stdin), read, read-rows (pruned + pushdown), sql, shell/REPL, branch/checkout/merge, history, ls, cat, gc/vacuum, journal-status/compact, version. `cat` (C20): full 64-char hash OR 6–63-char lowercase-hex prefix resolved against the blobs/ tree via a throwaway raw-listing handle (LocalFS / pure-URL S3) — works on prefixed S3 where the kernel's list_paths surfaces filter blob keys; one match → bytes, zero → historical error, many → ambiguity capped at 5 candidates; 0/1-char args error cleanly (was a store-layer panic). Uses clap. Single binary, DuckDB philosophy. |
| `cli/tests/cli_integration.rs` | 850 | 45 integration tests: init, write/read JSON, write from file/stdin, dedup, ls, branch+checkout/merge, cat by prefix, cat full hash / unique short prefix / ambiguity (deterministic birthday grind via pond_kernel::hash_bytes) / no-match / below-minimum gate / single-char clean error, SQL select/where/limit, shell REPL incl. \d unknown-collection + \! escape + SQL error resilience, write-rows/read-rows round trip, undo, persistence across invocations, version. All pass. |
